...
function add_product(id,name,productNumber) {
    ...
    // +`${e.target.dataset.productName}` + "/" + `${e.target.dataset.productId}/`;
...
function cartActionFunction(e) {
    ...
    let productQuantity = e.target.parentElement.parentElement.previousElementSibling;
    // productQuantity = e.target.parentElement.parentElement.parentElement.previousElementSibling;
    ...
    fetch(url, {
        ...
        body: JSON.stringify({ 'id': productId, 'name': productName, 'action': cartAction}),
        })
...
function nextFunction() {
    ...
    if (unshow >= next)  {
        ...
        // go to the next paginator element
        start = next;
        next += next
    }
    // if unshow is less than the element to display
        ...
        nextBtn.hidden = false;
        resetItemView();
        // if the current next apginator is more than item number iterate from start to items number because if next is more than item number we will have indexerror
        if (next > itemsNumber) {
